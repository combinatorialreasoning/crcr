...
class Sokoban(Environment[State, specs.DiscreteArray, Observation]):
    ...
    def step(
        ...
        if jnp.all(action == NOOP):
            next_variable_grid, next_agent_location = (state.variable_grid, state.agent_location)
        else:
            next_variable_grid, next_agent_location = self.move_agent(state.variable_grid, action, state.agent_location)
