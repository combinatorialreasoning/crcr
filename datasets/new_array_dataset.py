...
def get_dataset_stats(directory):
    ...
    for filename in os.listdir(directory):
        if filename.endswith('.pkl'):
            # arr = joblib.load(directory + '/' + filename)
            num_traj += 1 #len(arr)                
            # del arr
...
@gin.configurable
class GenDataset():
    def __init__(self, loggers, path, gamma, max_horizon, double_batch = False, two_sided_sampling = False, length = 20, noise_prob = 0., min_ind=None, max_ind=None, max_size=None, sampling_probability='uniform', device='cpu', weights=None):
        ...
        # assert double_batch in [True, False]
    ...
    def _read_next_file(self):
        ...
        # self.file_ind %= len(self.file_paths)
    ...
    def _get_trajs(self, n_traj):
        ...
        return self.buffer[self.buffer_ind - n_traj:self.buffer_ind]
            
            
        
    def _get_batch(self, batch_size):
        assert batch_size % self.double_batch == 0
        trajs = self._get_trajs(batch_size // self.double_batch)
        trajs = trajs.repeat(self.double_batch, 1, 1)
        ...
        elif self.sampling_probability == 'equalized':
        # Exponential sampling - more weight towards earlier timesteps
            weights = torch.pow(0.9, torch.arange(len(trajs[0]) - 1, dtype=torch.float32)) 
            i = torch.multinomial(weights, num_samples=len(trajs), replacement=True)

        elif self.sampling_probability == 'exp':
            # Exponential sampling - more weight towards earlier timesteps
            weights = torch.exp2(-torch.arange(len(trajs[0]) - 1, dtype=torch.float32))
        ...
        elif self.sampling_probability == 'custom':
            assert len(self.weights) == trajs.shape[1], f"Number of weights ({len(self.weights)}) does not match number of timesteps ({trajs.shape})"
            i = torch.multinomial(torch.tensor(self.weights).float(), num_samples=len(trajs), replacement=True)
        else:
        ...
        if self.two_sided_sampling:
            rev_probs = self.gamma ** (torch.arange(self.max_horizon).unsqueeze(0).repeat(len(trajs), 1).float() + 1)
            rev_mask = torch.arange(self.max_horizon).repeat(len(trajs), 1) <= (i.unsqueeze(1) - 1)
            rev_probs *= rev_mask.float()
            probs = torch.cat((rev_probs, probs), dim=1)
        ...
        if self.two_sided_sampling:
            delta[delta >= self.max_horizon] = delta[delta >= self.max_horizon] - self.max_horizon
            delta[delta < self.max_horizon] = -delta[delta < self.max_horizon] - 1
