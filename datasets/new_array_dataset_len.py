...
@gin.configurable
class GenDataset():
    def __init__(self, loggers, path, gamma, max_horizon, i_weights='none', double_batch = 1, two_sided_sampling = False, length = 20, noise_prob = 0., min_ind=None, max_ind=None, max_size=None, sampling_probability='uniform', device='cpu', weights=None):
    ...
    def _get_batch(self, batch_size):
        ...
        mask_2 = torch.arange(len(trajs[0])).unsqueeze(0).repeat(len(trajs), 1) < lens.unsqueeze(1).cpu() / 2
        mask_3 = torch.arange(len(trajs[0])).unsqueeze(0).repeat(len(trajs), 1) >= lens.unsqueeze(1).cpu() / 2
        ...
        if self.i_weights == 'front':
            weights[mask_2] = 2
        elif self.i_weights == 'back':
            weights[mask_3] = 2
        else:
            assert self.i_weights == 'none'
        ...
        return torch.concat((trajs[torch.arange(len(trajs)), i].unsqueeze(1), goals.unsqueeze(1)), axis=1).flatten(2, 3).to(torch.float32).to(self.device) # , trajs[:, i+delta, :])
